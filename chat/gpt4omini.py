...
class GPT4ominiClient:
    ...
    def chat_completion(self, prompt: str, stream: bool = False) -> Optional[str]:
        """
        ...
        response = result.invoke({"prompt": prompt})

        if not stream:
            return response.content.strip()

def gpt4omini(prompt: str, api_key: Optional[str] = None, stream: bool = False) -> Optional[str]:
    """
